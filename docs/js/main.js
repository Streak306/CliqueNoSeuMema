...
// MISSOES
// main.js - SISTEMA: Missões, Upgrades Passivos, Boosts

// ======================
// CONFIGURAÇÕES INICIAIS
// ======================
const SAVE_KEY = 'cliquenoseumema_save_v1';
let state = {
  meminha: 0,
  totalClicks: 0,
  mps: 0, // meminhas por segundo passivo
  clickValue: 1, // meminhas por clique
  upgradesOwned: {},
  passiveUpgrades: {},
  missions: {},
  activeBoost: null,
...
function fmt(n) {
  ...
  return `${Math.floor(n*10)/10}${units[i]}`;
}
...
// ======================
const MISSIONS_DEFS = [
  { id: 'm1', desc: 'Clique 500 vezes', tipo: 'clicks', alvo: 500, recompensa: 500 },
  { id: 'm2', desc: 'Ganhe 10.000 meminhas', tipo: 'mems', alvo: 10000, recompensa: 1500 },
  { id: 'm3', desc: 'Compre 5 upgrades', tipo: 'upgrades', alvo: 5, recompensa: 2500 },
];

// ======================
// DEFINIÇÕES DE UPGRADES PASSIVOS
// ======================
const PASSIVE_UPGRADES_DEFS = [
  { id: 'p1', nome: 'Bot dos Memas', preco: 1000, tipo: 'autoClicks', valor: 1, descricao: 'Gera 1 clique a cada 2s.' },
  { id: 'p2', nome: 'Editor Preguiçoso', preco: 5000, tipo: 'mults', mpsMult: 2, clickMult: 0.75, descricao: 'Dobra MPS, reduz clicks em 25%.' },
  { id: 'p3', nome: 'IA Memeira', preco: 15000, tipo: 'offlineGain', valor: 0.01, descricao: 'Gera 1% do total de meminhas por minuto offline.' },
];

// ======================
// DEFINIÇÕES DE BOOSTS
// ======================
const BOOSTS_DEFS = [
  { id: 'b1', nome: 'Modo Viral', mpsMult: 2, clickMult: 1, duracaoMs: 30000, preco: 2000 },
  { id: 'b2', nome: 'Memes em Alta', mpsMult: 1, clickMult: 5, duracaoMs: 15000, preco: 1500 },
  { id: 'b3', nome: 'Parceria Influencer', mpsMult: 1.5, clickMult: 1.5, duracaoMs: 60000, preco: 5000 },
];

// ======================
// UI: cria containers mínimos se faltar
// ======================
function ensureUI() {
  const existing = doc => document.querySelector(doc);
  if (!existing('#game-root')) {
  ...
  root.innerHTML = `
    <div id="hud" style="font-family:Arial, sans-serif; padding:10px;">
      <div style="display:flex; gap:12px; align-items:center;">
        <div><strong>Mepinha:</strong> <span id="meminha-count">0</span></div>
        <div><strong>MPS:</strong> <span id="mps-count">0</span></div>
        <div><strong>Valor por clique:</strong> <span id="click-value">1</span></div>
      </div>
      <div style="margin-top:8px;">
        <button id="big-click" style="padding:10px 20px; font-size:18px;">CLIQUE AQUI</button>
      </div>
      <div style="display:flex; gap:20px; margin-top:12px;">
        <div id="shop" style="min-width:280px; border:1px solid #444; padding:8px;">
          <h4>Loja / Passivos</h4>
          <div id="passive-list"></div>
        </div>
        <div id="missions" style="min-width:280px; border:1px solid #444; padding:8px;">
          <h4>Missões</h4>
          <div id="missions-list"></div>
        </div>
        <div id="boosts" style="min-width:240px; border:1px solid #444; padding:8px;">
          <h4>Boosts</h4>
          <div id="boosts-list"></div>
          <div id="boost-timer" style="margin-top:8px;"></div>
        </div>
      </div>
      <div style="margin-top:12px;">
        <button id="reset-save" style="padding:6px 8px;">Apagar Save</button>
      </div>
    </div>
  `;
}
...
function loadState() {
  ...
  if (raw) {
    try {
      ...
      state.passiveUpgrades = state.passiveUpgrades || {};
      state.missions = state.missions || {};
    } catch(e) {
      console.error('Erro ao carregar save:', e);
    }
  } else {
    // init missions and passives empty
    MISSIONS_DEFS.forEach(m => state.missions[m.id] = { prog: 0, completa: false, defin: m });
    PASSIVE_UPGRADES_DEFS.forEach(p => state.passiveUpgrades[p.id] = { qtd: 0, defin: p });
  }
}
...
// LÓGICA DE JOGO
// ======================
function giveMeminha(amount) {
  state.meminha += amount;
  state.totalClicks = state.totalClicks; // no change but keep consistent
  updateMissions('mems', amount);
  renderHUD();
  saveState();
}

function doClick(manual = true) {
  const clickGain = state.clickValue;
  state.meminha += clickGain;
  if (manual) state.totalClicks += 1;
  updateMissions('clicks', manual ? 1 : 0); // se auto clique, manual=false
  renderHUD();
  saveState();
}

function buyPassive(id) {
  const def = PASSIVE_UPGRADES_DEFS.find(p => p.id === id);
  if (!def) return;
  const owned = state.passiveUpgrades[id].qtd || 0;
  const price = Math.floor(def.preco * Math.pow(1.5, owned)); // escalonamento
  if (state.meminha < price) return alert('Meminha insuficiente.');
  state.meminha -= price;
  state.passiveUpgrades[id].qtd = owned + 1;
  // aplicar efeito imediato
  applyPassiveEffect(def, 1);
  updateMissions('upgrades', 1);
  renderAll();
  saveState();
}

function applyPassiveEffect(def, times=1) {
  // cada tipo tem efeito próprio
  if (def.tipo === 'autoClicks') {
    // aumenta número de cliques automáticos por unidade
    // (vamos contabilizar no tick)
    // aqui não precisa alterar mps diretamente
  } else if (def.tipo === 'mults') {
    state.mps *= def.mpsMult ** times;
    state.clickValue *= def.clickMult ** times;
  } else if (def.tipo === 'offlineGain') {
    // nada imediato; será considerado no calculo offline
  }
}

function computeMPS() {
  // base passa por upgrades
  let base = 0;
  // podemos definir mps básico baseado em owned passives: por exemplo, bot gera autoClicks -> convertemos para mps
  PASSIVE_UPGRADES_DEFS.forEach(def => {
    const owned = state.passiveUpgrades[def.id]?.qtd || 0;
    if (!owned) return;
    if (def.tipo === 'autoClicks') {
      // cada unidade gera 1 clique a cada 2s => 0.5 clicks/s * clickValue meminha
      base += owned * (1/2) * state.clickValue * (def.valor || 1);
    } else if (def.tipo === 'mults') {
      // multiplicador já aplicado ao state.mps quando comprado
    } else if (def.tipo === 'offlineGain') {
      // offline handled separately
    }
  });
  // apply active boost multiplier if exists
  if (state.activeBoost) {
    const boostDef = BOOSTS_DEFS.find(b => b.id === state.activeBoost);
    if (boostDef && boostDef.mpsMult) base *= boostDef.mpsMult;
  }
  state.mps = base;
  return base;
}
...
// MISSÕES
// ======================
function updateMissions(tipo, valor) {
  MISSIONS_DEFS.forEach(m => {
    const ms = state.missions[m.id];
    if (!ms || ms.completa) return;
    if (m.tipo === 'clicks' && tipo === 'clicks') {
      ms.prog = (ms.prog || 0) + valor;
    } else if (m.tipo === 'mems' && tipo === 'mems') {
      ms.prog = (ms.prog || 0) + valor;
    } else if (m.tipo === 'upgrades' && tipo === 'upgrades') {
      ms.prog = (ms.prog || 0) + valor;
    }
    if (ms.prog >= m.alvo) {
      ms.completa = true;
      // recompensa automática no claim para dar controle ao jogador:
      // aqui apenas marca completa; player deve clicar "coletar"
    }
  });
}
...
function claimMission(id) {
  const ms = state.missions[id];
  if (!ms || !ms.completa || ms.claimed) return;
  state.meminha += ms.defin.recompensa;
  ms.claimed = true;
  renderAll();
  saveState();
}
...
// BOOSTS
// ======================
function buyAndActivateBoost(id) {
  const def = BOOSTS_DEFS.find(b => b.id === id);
  if (!def) return;
  if (state.activeBoost) return alert('Já existe um boost ativo.');
  if (state.meminha < def.preco) return alert('Meminha insuficiente para o boost.');
  state.meminha -= def.preco;
  state.activeBoost = def.id;
  state.boostExpireAt = now() + def.duracaoMs;
  // aplicar efeitos são lidos por computeMPS e na hora do clique (visitados no tick)
  renderAll();
  saveState();
}
...
function checkBoostExpiration() {
  if (!state.activeBoost) return;
  if (now() >= state.boostExpireAt) {
    state.activeBoost = null;
    state.boostExpireAt = null;
    // recompute normal effects (re-render)
    renderAll();
    saveState();
  }
}
...
function gameTick() {
  const cur = now();
  const dt = (cur - state.lastTick) / 1000.0; // segundos
  state.lastTick = cur;

  // atualizar mps a cada tick
  computeMPS();
  if (state.mps > 0) {
    const gain = state.mps * dt;
    state.meminha += gain;
    updateMissions('mems', gain);
  }

  // auto clicks from bots: calcular cliques por segundo dos bots e converter
  // já considerados no computeMPS (convertendo clique->mems). Se quiser cliques explícitos para contador total:
  PASSIVE_UPGRADES_DEFS.forEach(def => {
    const owned = state.passiveUpgrades[def.id]?.qtd || 0;
    if (def.tipo === 'autoClicks' && owned) {
      // cada gera 0.5 clicks/s * owned -> transformar em mems (mas ja feito via computeMPS)
      // contar como cliques totais (estatística) cada segundo (aprox)
      // para simplicidade adicionamos contador de clicks baseado em rate
      // não incrementamos totalClicks para não inflar manual achievements
    }
  });
  ...
  if (cur % 10000 < 50) saveState();
}

function startLoop() {
  state.lastTick = now();
  setInterval(gameTick, 1000/2); // 2 ticks por segundo
}
...
function handleOfflineGain() {
  ...
  // calcular bonus baseado nas passives "offlineGain"
  const iaDef = PASSIVE_UPGRADES_DEFS.find(d => d.tipo === 'offlineGain');
  const owned = state.passiveUpgrades[iaDef?.id]?.qtd || 0;
  if (owned && iaDef) {
    // ganha 1% do total de meminhas por minuto por unidade (exemplo)
    const percent = iaDef.valor || 0.01;
    const gain = state.meminha * percent * owned * minutesAway;
    if (gain > 0) {
      state.meminha += gain;
      alert(`Bem-vindo de volta! Você ganhou ${fmt(gain)} meminhas offline.`);
      updateMissions('mems', gain);
    }
  }
}
...
// ======================
function renderHUD() {
  if (!doc('#meminha-count')) return;
  doc('#meminha-count').textContent = fmt(state.meminha);
  doc('#mps-count').textContent = fmt(state.mps);
  doc('#click-value').textContent = fmt(state.clickValue);
  // boost timer
  if (state.activeBoost && state.boostExpireAt) {
    const remain = Math.max(0, Math.ceil((state.boostExpireAt - now())/1000));
    doc('#boost-timer').textContent = `Boost ${state.activeBoost} ativo — ${remain}s`;
  } else {
    doc('#boost-timer').textContent = '';
  }
}

function renderPassives() {
  const list = doc('#passive-list');
  list.innerHTML = '';
  PASSIVE_UPGRADES_DEFS.forEach(def => {
    const owned = state.passiveUpgrades[def.id]?.qtd || 0;
    const price = Math.floor(def.preco * Math.pow(1.5, owned));
    const div = document.createElement('div');
    div.style.marginBottom = '8px';
    div.innerHTML = `<strong>${def.nome}</strong> x${owned} <div style="font-size:13px">${def.descricao}</div>
      <div style="margin-top:4px;"><button data-buy="${def.id}">Comprar (${fmt(price)})</button></div>`;
    list.appendChild(div);
    div.querySelector('button').addEventListener('click', () => buyPassive(def.id));
  });
}

function renderMissions() {
  const list = doc('#missions-list');
  list.innerHTML = '';
  MISSIONS_DEFS.forEach(m => {
    const ms = state.missions[m.id] || { prog:0, completa:false, claimed:false, defin: m};
    const prog = Math.min(ms.prog || 0, m.alvo);
    const pct = Math.floor((prog / m.alvo) * 100);
    const div = document.createElement('div');
    div.style.marginBottom = '8px';
    div.innerHTML = `<div><strong>${m.desc}</strong></div>
      <div style="font-size:13px">Progresso: ${Math.floor(prog)}/${m.alvo} (${pct}%)</div>
      <div style="margin-top:4px;">
        ${ms.completa && !ms.claimed ? `<button data-claim="${m.id}">Coletar ${fmt(m.recompensa)}</button>` : (ms.claimed ? '<em>Coletado</em>' : '')}
      </div>`;
    list.appendChild(div);
    const btn = div.querySelector('button');
    if (btn) btn.addEventListener('click', () => claimMission(m.id));
  });
}

function renderBoosts() {
  const list = doc('#boosts-list');
  list.innerHTML = '';
  BOOSTS_DEFS.forEach(b => {
    const div = document.createElement('div');
    div.style.marginBottom = '8px';
    div.innerHTML = `<div><strong>${b.nome}</strong> <div style="font-size:13px">Duração: ${b.duracaoMs/1000}s — Preço: ${fmt(b.preco)}</div>
      <div style="margin-top:4px;"><button data-boost="${b.id}">Ativar</button></div></div>`;
    list.appendChild(div);
    div.querySelector('button').addEventListener('click', () => buyAndActivateBoost(b.id));
  });
}
...
function init() {
  ...
  // load state
  loadState();
  // initialize missing mission/passive objects if new save
  MISSIONS_DEFS.forEach(m => {
    if (!state.missions[m.id]) state.missions[m.id] = { prog: 0, completa: false, claimed: false, defin: m };
    state.missions[m.id].defin = m;
  });
  PASSIVE_UPGRADES_DEFS.forEach(p => {
    if (!state.passiveUpgrades[p.id]) state.passiveUpgrades[p.id] = { qtd: 0, defin: p };
    state.passiveUpgrades[p.id].defin = p;
  });
  // UI bindings
  doc('#big-click').addEventListener('click', () => {
    // clique normal: aplicar boost de clique caso ativo
    let mult = 1;
    if (state.activeBoost) {
      const bd = BOOSTS_DEFS.find(b => b.id === state.activeBoost);
      if (bd && bd.clickMult) mult = bd.clickMult;
    }
    doClick(true);
    // aplicar click multiplier: multiplicar instantaneamente meminha gained
    state.meminha += (state.clickValue * (mult - 1)); // porque doClick já adicionou base
    renderAll();
    saveState();
  });
  doc('#reset-save').addEventListener('click', () => {
